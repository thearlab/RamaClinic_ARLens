...
//@ui {"widget":"group_start", "label":"Look", "hint" : "Set these based on Orthographic Camera units configured in Canvas component"}
//@input int visibleItems = 3 { "widget": "combobox", "values": [{"label": "3", "value": 3}, {"label": "5", "value": 5}]}
//@input float itemSize = 1.9 {"widget": "slider", "min": 0.2, "max": 500.0, "step": 0.01}
//@input float carouselRadius = 1.9 {"widget": "slider", "min": 0.2, "max": 600.0, "step": 0.01}
...
//@input float ringMargin = 0.2 {"widget": "slider", "min": 0, "max": 100.0, "step": 0.01, "showIf": "advanced"}
...
function initInteraction(root, st) {
    ...
    carouselUI.createEvent("TouchMoveEvent").bind(function (args) {
        if (!touching || touchId != args.getTouchId()) {
            return;
        }
...
function triggerBehaviors(behaviors) {
    ...
    if (behaviors && behaviors.trigger) {
        behaviors.trigger();
    } else {
        print("WARNING: please assign the Behavior Script Component");
    }
